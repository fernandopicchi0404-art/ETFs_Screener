#!/usr/bin/env python3
"""Extrai composição (N-PORT) dos ETFs e salva no banco central."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extrai composição SEC N-PORT dos ETFs.")
    parser.add_argument("--etf", action="append", help="Ticker específico (ex.: SCHY).")
    ...
    if args.all:
        ...
        etf_rows = list_etfs(db, priority=args.priority, tickers=tickers)
